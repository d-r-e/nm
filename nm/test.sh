...
if [ "$1" ] && [ "$1" != "--long" ];then
    diff -i <(./$BIN "${1}" 2>/dev/null| cat -e) <(nm --no-sort "${1}" 2>/dev/null| cat -e) >/dev/null
    if [ "$?" == "0" ]; then
        echo OK "->" $1
    else
        diff -y <(./$BIN "${1}" 2>/dev/null| cat -e) <(nm --no-sort "${1}" 2>/dev/null| cat -e) 
    fi
    exit 0
fi
...
FILES_O=$(find libft -name "*.o" 2>/dev/null )
...
function test_files()
{
    ...
    for f in $FILES;do
        ...
        if [ "$?" == "0" ]; then
            NFILES=$(($NFILES + 1))
            diff -i <(./$BIN "${f}" 2>/dev/null| cat -e) <(nm --no-sort "${f}" 2>/dev/null| cat -e) >/dev/null
            if [ "$?" == "0" ];then 
